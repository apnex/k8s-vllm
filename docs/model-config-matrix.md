...
| **G15** | **Qwen3-Coder-Next** (80B total / 3B active MoE) | **Q4_K_M GGUF** | **MoE expert offload to host RAM** — vLLM ≥0.15.0 native MoE offload OR llama.cpp `--n-gpu-layers` route; ~48 GB combined memory budget (32 GB GPU + ~16 GB CPU spill of cold experts) | **New SOTA-class SWE coder model** (Feb 2026; ~70.6% SWE-bench Verified per benchlm.ai). Hybrid Gated DeltaNet (linear, 75% of layers) + Gated Attention (full, 25%) — 512 experts, 10 routed per token. 256K native context. **Validates "use 128 GB host RAM for MoE expert offload" thesis** — only routed experts stream per token (cheap) vs dense weight streaming (catastrophic). External validation: compute-market.com reports 38–48 tok/s on RTX 5090 + 64 GB DDR5-6000 at full 256K context. Caveat: Q4_K_M is GGUF-native — vLLM supports loading GGUF but the matrix has been pure AWQ-INT4 until now; alternatively an AWQ-INT4 variant could be tried. | **new H — MoE expert offload + new model class** |
| **G16** | **Qwen3.6-27B** (27B dense) | **AWQ-INT4** (or FP8 variant; HF ships both) | **FA2 / bf16-kv / no-spec** — fits cleanly on 32 GB | **Top SWE-bench Verified candidate that fits this hardware** (77.2% per benchlm.ai — higher than every other locally-runnable open model, including Qwen3-Coder-Next at 70.6% and our current locked production at ~60%). Dense 27B, same hybrid Gated DeltaNet + Gated Attention architecture as Qwen3-Coder-Next (`full_attention_interval: 4`, GQA-2 in full layers). 256K native context (extensible to 1M). At INT4: ~13 GB weights → ~17–19 GB free for KV (massive context headroom). FP8 variant is 30.9 GB (tight on context). Apache-2.0. **Could supersede the locked production Qwen3-Coder-30B-A3B if SWE-quality holds and decode tok/s is competitive.** | **new H — dense hybrid-attention SWE candidate** |
...
| Newer Qwen3-Coder releases | actionable today | filed as G15 (Qwen3-Coder-Next) + G16 (Qwen3.6-27B dense, higher SWE-bench than Coder-Next) | **OPEN — top-priority quality candidates** |
