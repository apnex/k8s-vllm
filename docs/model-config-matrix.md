...
| **R15** | 2026-05-12 23:27 | **Qwen3.6-27B** (dense, hybrid linear+full attention) | AWQ-INT4 (cyankiwi) | FA2 / bf16-kv / no-spec / max_model_len=65536 / `--enforce-eager` (cudagraph profiler OOM forced eager) | **2/5 (40%)** | 259.89 | n/a | **~29** | n/a | 30.4 GB | **FALSIFIED — quality loss vs R11.** affine-cipher + beer-song pass; book-store + bottle-song + bowling fail. Web-research 77.2% SWE-bench Verified did NOT predict polyglot (different bench measures different abilities — SWE-bench uses agentic scaffolds, polyglot is single-shot). Eager-mode forced by Qwen3.6-27B multimodal vision tower (~5-10 GB unquantized) leaving no cudagraph profiling headroom. Decode 8.4× slower than R11 due to eager. **Parked pending FP8 KV + agentic-harness retest.** | `polyglot-20260512T232735Z-qwen3.6-27b-python/` | G16 (filed) |
| **R16** | 2026-05-13 00:48 | **Qwen3-Coder-Next** (80B/3B-active MoE, hybrid linear+full attention) | AWQ-INT4 (cyankiwi) | FA2 / bf16-kv / no-spec / max_model_len=65536 / `--enforce-eager` / **expert-offload tested 3 ways** | n/a (no polyglot possible) | n/a | n/a | **0.20 → 1.19 → CRASH** | n/a | ~31 GB | **FALSIFIED — vLLM 0.20.2 MoE offload not viable on TB4.** Three sub-attempts: (a) `--cpu-offload-gb=20` default → 0.20 tok/s (UVA streams all weights, not just experts); (b) `--cpu-offload-params=experts` → 1.19 tok/s (6× win but still ~210× slower than R11); (c) `--offload-backend=prefetch` → crashes on AWQ `g_idx` pin assertion at runtime even with `VLLM_WEIGHT_OFFLOADING_DISABLE_PIN_MEMORY=1`. **Verified: nightly is byte-identical** to v0.20.2 on offload modules (md5 confirmed) — no MoE-aware path merged. PR #37190 (LFRU expert cache + `--moe-expert-cache-size`) is open but unmerged. **Parked pending PR #37190 merge.** | (no polyglot archive) | G15 (filed) |
...
| **R15 (Qwen3.6-27B Dense / FA2 / eager / 65k)** | ✓ (7145 tok) | ✓ (4176 tok) | ✗ (8636 tok) | ✗ (3343 tok) | ✗ (13778 tok) |
...
| ~~G15~~ | ~~Qwen3-Coder-Next~~ (80B/3B MoE) | ~~AWQ-INT4 / GGUF Q4_K_M~~ | ~~MoE expert offload to host RAM~~ | **TESTED — see R16 (FALSIFIED).** vLLM 0.20.2 + nightly both lack the MoE-aware expert offload path; only UVA (page-fault, ~1 tok/s on TB4) and prefetch (crashes on AWQ `g_idx` pin) are available. Verified byte-identical offload modules between v0.20.2 and nightly (md5 confirmed). **PARKED — see G17 watch.** | ~~MoE offload~~ |
| ~~G16~~ | ~~Qwen3.6-27B~~ (27B dense) | ~~AWQ-INT4 / FP8~~ | ~~FA2 / bf16-kv / no-spec~~ | **TESTED — see R15 (FALSIFIED on n=5 polyglot, 2/5).** Web-research SWE-bench 77.2% did not predict our polyglot result; different benchmarks measure different abilities. Eager-mode forced by multimodal vision tower OOM at cudagraph profiling. **DEFERRED for FP8 retest** — see G18 below. | ~~dense hybrid-attention SWE candidate~~ |
| **G17** | Qwen3-Coder-Next (80B/3B MoE) | AWQ-INT4 (cyankiwi) — same as R16 | Wait for **vLLM PR #37190** ([github](https://github.com/vllm-project/vllm/pull/37190)) to merge: adds `--moe-expert-cache-size` + CPU-pinned expert weights + GPU LFRU cache + cross-layer temporal prediction. When merged, retry G15 config with the new flag. | **The single highest-value external event for this model class.** PR is open with verified label as of 2026-05-11; ~980 LOC; CI passing; awaiting code-owner review. Tracking it converts G15 from "blocked" to "actionable" the day it merges. Expected unlock: 5-20 tok/s (still slower than R11 but viable for polyglot). | external (track) |
| **G18** | Qwen3.6-27B (27B dense) | AWQ-INT4 (cyankiwi) — same as R15 | FA2 / **fp8 KV** / no-spec / max_model_len=96k / cudagraphs (drop eager) + **retry-with-feedback polyglot harness** | Retry G16 with: (a) FP8 KV halves attention memory → may regain cudagraphs headroom that eager mode took; (b) retry-with-test-feedback polyglot matches Aider's actual benchmark methodology that the model was tuned for (R15's 2/5 single-shot was unfair to a reasoning-tuned model). Cheap if both arrive together. | future quality retest |
...
| Newer Qwen3-Coder releases | both tested 2026-05-12/13 | **G15 FALSIFIED** (R16 — vLLM MoE offload immature on TB4; PARKED pending PR #37190); **G16 FALSIFIED on n=5 polyglot** (R15 — 2/5; PARKED pending FP8 KV + retry-with-feedback retest as G18). Locked production R11 remains best. | results in, **PARKED for upstream events** |
