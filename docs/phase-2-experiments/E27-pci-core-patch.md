...
```bash
sudo /root/k8s-vllm/tools/get-pci-stats.sh --baseline E27-cold-control
# Enter broken-BAR1 state per recipe
sudo /root/k8s-vllm/tools/get-pci-stats.sh --snapshot E27
sudo /root/k8s-vllm/tools/get-pci-stats.sh --diff E27
```
...
**Status:** BLOCKED — kernel build env + deep PCI core analysis not yet done
