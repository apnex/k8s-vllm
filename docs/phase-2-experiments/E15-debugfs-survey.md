...
```bash
sudo /root/k8s-vllm/tools/get-pci-stats.sh --baseline E15
```
...
sudo /root/k8s-vllm/tools/get-pci-stats.sh --snapshot E15-after-$(basename "$ENTRY")
sudo /root/k8s-vllm/tools/get-pci-stats.sh --diff E15-after-$(basename "$ENTRY")
...
```bash
sudo /root/k8s-vllm/tools/get-pci-stats.sh --snapshot E15
sudo /root/k8s-vllm/tools/get-pci-stats.sh --diff E15
```
...
**Per-entry results:** (filled in during run)

| Entry | Pre-value | Post-value | Diff effect |
|---|---|---|---|
| | | | |
