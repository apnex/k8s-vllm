...
```bash
sudo /root/k8s-vllm/tools/get-pci-stats.sh --baseline E03
```
...
```bash
sudo /root/k8s-vllm/tools/get-pci-stats.sh --snapshot E03
sudo /root/k8s-vllm/tools/get-pci-stats.sh --diff E03
```
...
```
(append per-file BAR1 readings here as enumeration proceeds)
```
